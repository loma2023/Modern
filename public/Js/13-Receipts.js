  ...
  if (input.classList.contains("Person")) {
    Customers.forEach(Customer => {
      if (Customer.Name.includes(input.value)) {
        SelectItmes.innerHTML += `<h3 onclick="SetName()" id="${Customer._id}">${Customer.Name}</h3>`
      }
    });
  } else if (input.classList.contains("Product")) {
    Products.forEach((Product, index) => {
      if (Product.Name.includes(input.value)) {
        SelectItmes.innerHTML += ` <h3 class="Part1" onclick="GetUnitsAndPriceItem(event)" index="${index}" id="${Product._id}">${Product.Name}</h3>`
      }
    });
    SetName()
  }
